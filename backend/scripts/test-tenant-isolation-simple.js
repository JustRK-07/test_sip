 ...
 * Tests campaign and lead isolation without needing to create tenants
 */
...
async function runTests() {
  ...
  console.log(`\n   Using Tenant A ID: ${tenantA}`);
  console.log(`   Using Tenant B ID: ${tenantB}`);

  try {
    // Step 1: Create campaigns for each tenant
    console.log('\n📋 Step 1: Creating campaigns for each tenant...\n');
    ...
    if (!campaignARes.data?.data?.id || !campaignBRes.data?.data?.id) {
      console.error('❌ Failed to create campaigns. Aborting tests.');
      console.error('Campaign A Response:', campaignARes.data);
      console.error('Campaign B Response:', campaignBRes.data);
      return;
    }
    ...
    // Step 2: Test campaign isolation
    console.log('\n📋 Step 2: Testing campaign isolation...\n');
    ...
    const hasOnlyOwnCampaigns =
      tenantACampaigns.length > 0 && tenantACampaigns.every((c) => c.tenantId === tenantA);
    ...
    const tenantBHasOnlyOwn =
      tenantBCampaigns.length > 0 && tenantBCampaigns.every((c) => c.tenantId === tenantB);
    ...
    console.log('\n📋 Step 3: Adding leads to campaigns...\n');
    ...
    // Step 4: Test lead isolation
    console.log('\n📋 Step 4: Testing lead isolation...\n');
    ...
    logTest(
      ...
      leadsAreIsolated && tenantALeads.length >= 2,
    ...
    logTest(
      ...
      leadsBIsolated && tenantBLeads.length >= 2,
    ...
    logTest(
      ...
      `Status: ${unauthorizedLeadRes.status} (expected 404 - campaign not found for this tenant)`
    );

    // Step 5: Test lead stats isolation
    console.log('\n📋 Step 5: Testing lead stats isolation...\n');
